    ...
    componentDidMount(){
        if(this.state.favoritos !==null){
            let storage = JSON.parse(this.state.favoritos)
                ...
                storage.map((elm =>
                    fetch(`https://api.themoviedb.org/3/movie${elm}?api_key=e4c9de46fa2b077570ba8601a80bf4d6`)
                    .then(res => res.json())
        ...
        return(
            <div>
                {this.state.pelis !== null ? 
                    ...
                    this.state.pelis.map(( elm, idx) => <TarjetaPelicula favorito={this.state.favoritos.includes(elm.id)}  actualizarFav={(array) => this.actualizarFav(array)}data= {elm}  key={idx + elm.title}/>)
